...
with mlflow.start_run(run_name="bert_classification_run") as run:
    ...
    # Logar o tokenizer como artefato
    tokenizer.save_pretrained("tokenizer")
    mlflow.log_artifacts("tokenizer")
    
    # Logar os resultados de avaliação
    with open("eval_results.txt", "w") as f:
        for key, value in eval_results.items():
            f.write(f"{key}: {value}\n")
    mlflow.log_artifact("eval_results.txt")
